...
class DownloadAlertState extends State<DownloadAlert> {
  ...
  Future<void> createFile() async {
    ...
    final String dirPath = path.join(appDocDir!.path, LanguageConstant.appName);
    if (Platform.isAndroid) {
      Directory(appDocDir.path.split('Android')[0] + LanguageConstant.appName)
          .createSync();
    } else {
    ...
    final String filePath = Platform.isAndroid
        ? path.join(
            ...
            LanguageConstant.appName,
            '$fileName.pdf',
          )
        : path.join(dirPath, '$fileName.pdf');
